...
function drawCurrentLocation() {
  if (navigator.geolocation) navigator.geolocation.getCurrentPosition(centerOnPosition, function (error) {
...
function update() {
  ...
  drawCurrentLocation();
  $.get('/locations/' + selectedLocation.shorthand, function (location) {
    if (typeof location == "string") location = JSON.parse(location);
    if (!location.scan) return;

    if (location.scan.status == "scanning" || location.scan.status == "scanned") {
      if (scanCircles.length > 0) {
        scanIntervals.forEach(function (interval) {
          clearInterval(interval);
          itntn = itntn - 1;
        });
        scanCircles.forEach(function (circle) {
          circle.setMap(null);
          circle = null;
        });
  ...
  $.get("/locations/" + selectedLocation.shorthand + "/pokemon" + (getParameterByName("this_will_fuck_up_your_browser_theres_like_600000_pokemon") != null ? "/all" : ""), function (allPokemon) {
    pokemonValues = Object.keys(pokemans).map(function (key) {
      return pokemans[key];
    });

    _.filter(pokemonValues, function (pokevalue) {
    ...
    allPokemon.forEach(function (pokemon) {
      if (pokemans[pokemon.pokekey]) return true;
      ...
      if (Date.now() - lastSelectionTime > 10000) notify_pokemon(pokemon.pokemon_name, pokemon.pokemon_data.pokemon_id, selectedLocation.shorthand);

      var infoboxes = [];
      var infobox;
      var interval;
      google.maps.event.addListener(marker, 'click', function () {
        var contentString = "<div class=\"card text-xs-center\" id=\"" + pokemon.pokekey + "\">" + '<div class="card-block">' + ("<h4 class=\"card-title\">" + pokemon.pokemon_name + "</h4>") + ("<h6 class=\"card-subtitle text-muted\">Despawns roughly in <span class=\"minutes\">" + Math.round((new Date(pokemon.hides_at * 1000) - Date.now()) / 1000 / 60, 1) + "</span> minutes</h6>") + ("<h8 class=\"card-subtitle text-muted\"><span class=\"seconds\">" + Math.round((new Date(pokemon.hides_at * 1000) - Date.now()) / 1000, 1) + "</span> seconds</h6>") + '</div>' + '<img src="/images/pokemon_sprites/' + pokemon.pokemon_data.pokemon_id + ".png" + '" alt="Card image cap">' + '<div class="card-block">' + ("<p class=\"card-text\">Spawnpoint ID: " + pokemon.spawn_point_id + "</p>") + '</div>' + '<div class="card-footer text-muted">' + 'More data to come' + '</div>' + '</div>';
...
function updateLocations() {
  return $.get('/locations/all/', function (res) {
    locations = res;
    var cookieLocation = _.find(locations, function (loc) {
      return loc.location.shorthand == readCookie("pokescanner-lastSelectedLocation");
    });
  ...
  function initialize() {
    ...
    updateLocations().then(function () {
      ...
      $('#search input').keyup(function (e) {
        ...
        var foundLocation = _.find(locations, function (loc) {
          return loc.location.shorthand.toLowerCase() == $(e.currentTarget).val().toLowerCase();
        });
        if (foundLocation) {
...
function selectLocation(location) {
  if (location.originalEvent instanceof Event) location = null;
  ...
  var thisLocation = _.find(locations, function (loc) {
    return loc.location.shorthand === selectedShorthand;
  });
